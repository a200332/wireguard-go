...
type ringBuffer struct {
	packets    [packetsPerRing]ringPacket
	head, tail uint32
	id         winrio.BufferId
	iocp       windows.Handle
	isFull     bool
...
func (ring *ringBuffer) Open() error {
	var err error
	ring.id, err = winrio.RegisterPointer(unsafe.Pointer(&ring.packets[0]), uint32(unsafe.Sizeof(ring.packets)))
	if err != nil {
		return err
	}
...
func (bind *afWinRingBind) Receive(buf []byte, isOpen *uint32) (int, Endpoint, error) {
	...
	if count == 0 {
		...
		var bytes uint32
		var key [2]uint32 // XXX: fix bug in x/sys/windows - this should be a uintptr
		var overlapped *windows.Overlapped
		err = windows.GetQueuedCompletionStatus(bind.rx.iocp, &bytes, &key[0], &overlapped, windows.INFINITE)
		if err != nil {
			return 0, nil, err
		}
		if atomic.LoadUint32(isOpen) != 1 {
...
func (bind *afWinRingBind) Send(buf []byte, nend *WinRingEndpoint, isOpen *uint32) error {
	...
	if count == 0 && bind.tx.isFull {
		...
		var key [2]uint32 // XXX: fix bug in x/sys/windows - this should be a uintptr
		var overlapped *windows.Overlapped
		err = windows.GetQueuedCompletionStatus(bind.tx.iocp, &bytes, &key[0], &overlapped, windows.INFINITE)
		if err != nil {
			return err
		}
		if atomic.LoadUint32(isOpen) != 1 {
